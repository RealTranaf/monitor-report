...
**Setup website dashboard**

Sau khi đã setup được để lấy dữ liệu sensor từ API, có thể đưa dữ liệu đó lên một website, frontend để dễ dàng kiểm tra data. Có 2 cách thường dùng để tạo dashboard: tạo một website HTML đơn giản hoặc sử dụng một dịch vụ tạo dashboard như Grafana.

**Tạo website HTML đơn giản**

Trong hệ thống đang chạy Openclaw, có thể setup một website gọn nhẹ sử dụng 1 proxy NodeJS để thực hiện call API PRTG và một file HTML static để show data đó trên web. Có thể sử dụng docker kèm web server như nginx để đóng gói quy trình chạy website này.

Một dashboard demo:

![](images/4-1.png)

HTML có thể sử dụng script JS để tự động refresh 60 giây 1 lần.

Default thì website có thể được access bởi các thiết bị cùng network local hay tailscale qua địa chỉ IP private. Để truy cập từ các thiết bị khác, cần mở các port cần thiết để chạy website qua public IP hoặc sử dụng domain name.

Nếu cần lưu lại dữ liệu để phục vụ mục đích phân tích về sau, nên tạo một backend hoàn chỉnh có kết nối với database.
